...
fn ldfss_flux(q_l: &[f64], q_r: &[f64]) -> [f64; 3] {
...
fn main() -> std::io::Result<()> {
    ...
    while t < T_FINAL {
        ...
        let mut fluxes = vec![[0.0; 3]; N_CELLS + 1];
        ...
        for i in 0..N_CELLS-1 {
            ...
            fluxes[i+1] = ldfss_flux(u_l.as_slice().unwrap(), u_r.as_slice().unwrap());
        }
        ...
        fluxes[0] = ldfss_flux(u.row(0).as_slice().unwrap(), u.row(0).as_slice().unwrap());
        fluxes[N_CELLS] = ldfss_flux(u.row(N_CELLS-1).as_slice().unwrap(), u.row(N_CELLS-1).as_slice().unwrap());
